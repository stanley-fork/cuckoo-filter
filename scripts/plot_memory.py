...
def main():
    ...
    if memory_data:
        ...
        benchmark_names = sorted(memory_data.keys(), key=get_last_memory_value, reverse=True)
        for bench_name in benchmark_names:
            sizes = sorted(memory_data[bench_name].keys())
            memory = [memory_data[bench_name][size] for size in sizes]

            ax1.plot(
                sizes, memory, "o-", label=bench_name, linewidth=2, markersize=6
            )
    ...
    if bytes_per_item_data:
        ...
        benchmark_names = sorted(bytes_per_item_data.keys(), key=get_last_bpi_value, reverse=True)
        for bench_name in benchmark_names:
            sizes = sorted(bytes_per_item_data[bench_name].keys())
