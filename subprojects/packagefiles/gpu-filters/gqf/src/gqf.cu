...
__host__ __device__ static inline void qf_dump_block(const QF* qf, uint64_t i) {
    ...
    printf(
        "Block %llu Runs from %llu to %llu\n",
        i,
        i * QF_SLOTS_PER_BLOCK,
        (i + 1) * QF_SLOTS_PER_BLOCK
    );
...
__host__ __device__ static inline uint64_t get_slot(const QF* qf, uint64_t index) {
    ...
    return (uint64_t)(((*p) >> (((index % QF_SLOTS_PER_BLOCK) * QF_BITS_PER_SLOT) % 8)) &
...
__host__ __device__ static inline uint64_t get_slot(const QF* qf, uint64_t index) {
     ...
     * to generate buggy code.  :/  */
    uint64_t* p = (uint64_t*)&get_block(qf, index / QF_SLOTS_PER_BLOCK)
                      ->slots[(index % QF_SLOTS_PER_BLOCK) * qf->metadata->bits_per_slot / 8];
...
__host__ __device__ static inline uint64_t run_end(const QF* qf, uint64_t hash_bucket_index) {
    ...
    uint64_t bucket_intrablock_rank =
        bitrank(get_block(qf, bucket_block_index)->occupieds[0], bucket_intrablock_offset);
    ...
    uint64_t runend_block_offset =
        bitselectv(get_block(qf, runend_block_index)->runends[0], runend_ignore_bits, runend_rank);
    if (runend_block_offset == QF_SLOTS_PER_BLOCK) {
        if (bucket_blocks_offset == 0 && bucket_intrablock_rank == 0) {
            ...
            do {
                runend_rank -=
                    popcntv(get_block(qf, runend_block_index)->runends[0], runend_ignore_bits);
                runend_block_index++;
                runend_ignore_bits = 0;
                runend_block_offset = bitselectv(
                    get_block(qf, runend_block_index)->runends[0], runend_ignore_bits, runend_rank
                );
...
// printf("slot %llu, slot_offset %02lx, block offset %llu, occupieds: %d ", slot_index,
// slot_offset, boffset, popcnt(occupieds));
#if DEBUG_ASSERTS
...
// that can be captured to finalize the test instead.
        ...
        // if (t < 0) 	throw std::out_of_range("next free slot is before current slot, either final
        // slot or gqf corruption.\n");
...
// exact same function as above, but forced to be host exclusive so that a try_catch statement in
// cluster counting will succeed.
    ...
    do {
        ...
        if (t < 0)
            throw std::out_of_range(
                "next free slot is before current slot, either final slot or gqf corruption.\n"
            );
...
__host__ __device__ static inline uint64_t shift_into_b(
    const uint64_t a,
    const uint64_t b,
    const int bstart,
    const int bend,
...
shift_remainders(QF* qf, uint64_t start_index, uint64_t empty_index) {
    ...
    #ifdef __CUDA_ARCH__
        gpu_memmove(
            &get_block(qf, empty_block)->slots[1],
    ...
    #ifdef __CUDA_ARCH__
    gpu_memmove(
        &get_block(qf, empty_block)->slots[start_offset + 1],
...
shift_remainders_cooperative(QF* qf, uint64_t start_index, uint64_t empty_index, int warpID) {
    uint64_t start_block = start_index / QF_SLOTS_PER_BLOCK;
    uint64_t start_offset = start_index % QF_SLOTS_PER_BLOCK;
    uint64_t empty_block = empty_index / QF_SLOTS_PER_BLOCK;
    uint64_t empty_offset = empty_index % QF_SLOTS_PER_BLOCK;
...
__host__ __device__ static inline void
shift_remainders(QF* qf, const uint64_t start_index, const uint64_t empty_index) {
    ...
    *REMAINDER_WORD(qf, last_word) =
        shift_into_b(0, *REMAINDER_WORD(qf, last_word), bstart, bend, qf->metadata->bits_per_slot);
}

#endif

__host__ __device__ static inline void
find_next_n_empty_slots(QF* qf, uint64_t from, uint64_t n, uint64_t* indices) {
    while (n) {
...
__host__ __device__ static inline void
shift_slots(QF* qf, int64_t first, uint64_t last, uint64_t distance) {
    int64_t i;
    if (distance == 1)
...
__host__ __device__ static inline void
shift_runends(QF* qf, int64_t first, uint64_t last, uint64_t distance) {
#if DEBUG_ASSERTS
    ...
    METADATA_WORD(qf, runends, 64 * last_word) =
        shift_into_b(0, METADATA_WORD(qf, runends, 64 * last_word), bstart, bend, distance);
}
...
remove_replace_slots_and_shift_remainders_and_runends_and_offsets(
    ...
    if (is_runend(qf, overwrite_index + old_length - 1)) {
        if (total_remainders > 0) {
            // If we're not deleting this entry entirely, then it will still the last entry in this
            // run
            METADATA_WORD(qf, runends, overwrite_index + total_remainders - 1) |=
...
encode_counter(QF* qf, uint64_t remainder, uint64_t counter, uint64_t* slots) {
...
__host__ __device__ static inline uint64_t
decode_counter(const QF* qf, uint64_t index, uint64_t* remainder, uint64_t* count) {
    uint64_t base;
    uint64_t rem;
    uint64_t cnt;
    uint64_t digit;
...
__host__ __device__ static inline qf_returns
insert1_if_not_exists(QF* qf, __uint64_t hash, uint64_t* value) {
    ...
    */
    ...
    if (is_empty(
        ...
        if (is_occupied(qf, hash_bucket_index)) {
            ...
            while (current_remainder < compare_remainder && runstart_index <= runend_index) {
                runstart_index++;
        ...
        if (operation >= 0) {
            ...
            if (empty_slot_index >= qf->metadata->xnslots) {
                printf(
                    "Ran out of space. Total xnslots is %lu, first empty slot is %lu\n",
                    qf->metadata->xnslots,
                    empty_slot_index
                );
                return QF_FULL;
            ...
            for (i = hash_bucket_index / QF_SLOTS_PER_BLOCK + 1;
                 i <= empty_slot_index / QF_SLOTS_PER_BLOCK;
                 i++) {
...
__device__ static inline qf_returns
insert1_if_not_exists_cooperative(QF* qf, __uint64_t hash, uint64_t* value, int warpID) {
    uint64_t hash_remainder = hash & BITMASK(qf->metadata->bits_per_slot);
    uint64_t hash_bucket_index = hash >> qf->metadata->bits_per_slot;
    uint64_t hash_bucket_block_offset = hash_bucket_index % QF_SLOTS_PER_BLOCK;
    ...
    */
    ...
    if (is_empty(
        ...
        if (is_occupied(qf, hash_bucket_index)) {
            ...
            uint64_t my_current_remainder =
                get_slot(qf, my_runstart_index) >> qf->metadata->value_bits;

            while (true) {
                // generate ballot
                bool ballot =
                    !((my_runstart_index <= runend_index) &&
                      (my_current_remainder < compare_remainder));
        ...
        if (operation >= 0) {
            ...
            if (warpID == 0)
                empty_slot_index = find_first_empty_slot(qf, runend_index + 1);
...
__host__ __device__ static inline qf_returns
insert1(QF* qf, __uint64_t hash, uint8_t runtime_lock) {
    // int ret_distance = 0;
    uint64_t hash_remainder = hash & BITMASK(qf->metadata->bits_per_slot);
    uint64_t hash_bucket_index = hash >> qf->metadata->bits_per_slot;
    uint64_t hash_bucket_block_offset = hash_bucket_index % QF_SLOTS_PER_BLOCK;
        ...
        if (is_occupied(qf, hash_bucket_index)) {
            ...
            if (runstart_index > runend_index) {
                ...
                if (insert_index == runstart_index &&
                    (carry > 0 || (current_remainder != 0 && digit >= current_remainder))) {
                    operation = 2; /* insert */
                    insert_index = runstart_index + 1;
                    if (!carry) /* To prepend a 0 before the counter if the MSD is greater than the
                                   rem */
                        new_value = 0;
                    else if (carry) { /* Increment the new value because we don't use 0 to encode
                                         counters */
                        new_value = 2;
...
insert1_cooperative(QF* qf, __uint64_t hash, uint8_t runtime_lock, int warpID) {
    int ret_distance = 0;
    uint64_t hash_remainder = hash & BITMASK(qf->metadata->bits_per_slot);
    uint64_t hash_bucket_index = hash >> qf->metadata->bits_per_slot;
    uint64_t hash_bucket_block_offset = hash_bucket_index % QF_SLOTS_PER_BLOCK;
...
__host__ __device__ static inline qf_returns
insert(QF* qf, __uint64_t hash, uint64_t count, uint8_t runtime_lock) {
    // int ret_distance = 0;
    uint64_t hash_remainder = hash & BITMASK(qf->metadata->bits_per_slot);
    uint64_t hash_bucket_index = hash >> qf->metadata->bits_per_slot;
    uint64_t hash_bucket_block_offset = hash_bucket_index % QF_SLOTS_PER_BLOCK;
        ...
        if (!is_occupied(qf, hash_bucket_index)) { /* Empty bucket, but its slot is occupied. */
            ...
            if (current_remainder < hash_remainder) {
                ...
                ret = insert_replace_slots_and_shift_remainders_and_runends_and_offsets(
                    qf,
                    1,
                    /* Append to bucket */ hash_bucket_index,
                    current_end + 1,
                    p,
                    &new_values[67] - p,
                    0
                ...
                uint64_t* p =
                    encode_counter(qf, hash_remainder, current_count + count, &new_values[67]);
                ret = insert_replace_slots_and_shift_remainders_and_runends_and_offsets(
                    qf,
...
__host__ __device__ inline static int
_remove(QF* qf, __uint64_t hash, uint64_t count, uint8_t runtime_lock) {
    ...
    */
    ...
    uint64_t* p = encode_counter(
        qf, hash_remainder, count > current_count ? 0 : current_count - count, &new_values[67]
    );
...
__host__ uint64_t qf_init(
    QF* qf,
    uint64_t nslots,
    uint64_t key_bits,
    uint64_t value_bits,
    enum qf_hashmode hash,
    uint32_t seed,
    void* buffer,
    uint64_t buffer_len
) {
...
#ifdef LOG_WAIT_TIME
    qf->runtimedata->wait_times =
        (wait_time_data*)calloc(qf->runtimedata->num_locks + 1, sizeof(wait_time_data));
...
__host__ bool qf_malloc(
    ...
    uint64_t init_size =
        qf_init(qf, nslots, key_bits, value_bits, hash, seed, buffer, total_num_bytes);
...
__host__ void qf_malloc_device(QF** qf, int nbits, int rbits, int vbits, bool bulk_config) {
    ...
    if (bulk_config) {
        ...
        cudaMalloc(
            (void**)&dev_locks, host_qf.runtimedata->num_locks * LOCK_DIST * sizeof(uint16_t)
        );
    ...
    cudaMemcpy(
        _blocks, host_qf.blocks, qf_get_total_size_in_bytes(&host_qf), cudaMemcpyHostToDevice
    );
...
__host__ __device__ qf_returns qf_insert_not_exists(
    QF* qf,
    uint64_t key,
    uint64_t value,
    uint64_t count,
    ...
    */
    ...
    if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
        if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
            key = aMurmurHash64A(((void*)&key), sizeof(key), qf->metadata->seed) %
                  qf->metadata->range;
...
__device__ qf_returns qf_insert_not_exists_cooperative(
    QF* qf,
    uint64_t key,
    uint64_t value,
    uint64_t count,
...
__host__ __device__ qf_returns
qf_insert(QF* qf, uint64_t key, uint64_t value, uint64_t count, uint8_t flags) {
    // We fill up the CQF up to 95% load factor.
    // This is a very conservative check.
...
__device__ void lock_16(uint16_t* lock, uint64_t index) {
    ...
    while (atomicCAS(
               (unsigned short int*)&lock[index * LOCK_DIST],
...
//  1) Set all offsets to keys here based on relative offset + keys - skips the launch call later -
//  TODO: double check that (keys + offset) - keys == offset. -- cpp says this works 2) subtract
//  sets of keys from each other to get the relative offsets - these will give offsets, last key
//  needs to subtract from origin pointer this means that the keys here are set to point to the
//  START of their bucket
__global__ void set_buffers_binary(QF* qf, uint64_t num_keys, uint64_t* keys, uint8_t flags) {
    uint64_t idx = threadIdx.x + blockDim.x * blockIdx.x;

    if (idx >= qf->runtimedata->num_locks)
        return;
...
__global__ void insert_from_buffers_hashed_values(QF* qf, uint64_t evenness, uint64_t* values) {
    ...
    for (uint64_t i = 0; i < my_count; i++) {
        int ret =
            qf_insert(qf, buffers[idx][i], values[my_offset + i], 1, QF_NO_LOCK | QF_KEY_IS_HASH);
...
__global__ void insert_from_buffers_cooperative(QF* qf, uint64_t evenness) {
    ...
    for (uint64_t i = 0; i < my_count; i++) {
        ...
        qf_returns ret_val = qf_insert_not_exists_cooperative(
            qf, buffers[idx][i], 0, 1, QF_NO_LOCK | QF_KEY_IS_HASH, &query, warpID
        );
...
__global__ void insert_from_buffers_thrust(
    QF* qf,
    uint64_t evenness,
    uint64_t* keys,
    uint64_t* vals,
    ...
    for (uint64_t i = 0; i < my_count; i++) {
        ...
        int ret = qf_insert(
            qf, buffers[idx][i], 0, vals[absolute_offset + i], QF_NO_LOCK | QF_KEY_IS_HASH
        );
...
__global__ void delete_from_buffers_hashed_vals(QF* qf, uint64_t evenness, uint64_t* vals) {
    ...
    for (uint64_t i = my_count; i >= 1; i--) {
        int ret = qf_remove(
            qf, buffers[idx][i - 1], vals[offset + i - 1], 1, QF_NO_LOCK | QF_KEY_IS_HASH
        );
...
__device__ qf_returns point_insert_not_exists(
    QF* qf,
    uint64_t key,
    uint64_t value,
    uint64_t& returnedVal,
    uint8_t flags
) {
    uint64_t query;
    ...
    while (true) {
        if (try_lock_16(qf->runtimedata->locks, lock_index)) {
            ...
            qf_returns ret =
                qf_insert_not_exists(qf, hash, value, 1, QF_NO_LOCK | QF_KEY_IS_HASH, &query);
...
__device__ qf_returns point_insert_not_exists_cooperative(
    QF* qf,
    uint64_t key,
    uint64_t value,
    uint64_t& returnedVal,
    ...
    while (true) {
        if (try_lock_16_coop(qf->runtimedata->locks, lock_index, warpID)) {
            ...
            qf_returns ret = qf_insert_not_exists_cooperative(
                qf, hash, value, 1, QF_NO_LOCK | QF_KEY_IS_HASH, &query, warpID
            );
...
__device__ uint64_t
point_query_concurrent(QF* qf, uint64_t key, uint64_t& returnedVal, uint8_t flags) {
    if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
        if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
            key = aMurmurHash64A(((void*)&key), sizeof(key), qf->metadata->seed) %
                  qf->metadata->range;
...
__global__ void bulk_get_cooperative(QF* qf, uint64_t* hashes, uint64_t nitems, uint64_t* counter) {
    ...
    if (itemID >= qf->runtimedata->num_locks)
        return;
    ...
    for (uint64_t i = warpID; i < my_count; i += 32) {
        ...
        if (point_query(
                qf, buffers[itemID][i] % qf->metadata->range, query, QF_NO_LOCK | QF_KEY_IS_HASH
            ) == 0) {
            // atomicAdd((unsigned long long int *) counter, (unsigned long long int) 1);
        }
...
__host__ uint64_t cooperative_bulk_get_wrapper(QF* qf, uint64_t* hashes, uint64_t nitems) {
    ...
    set_buffers_binary<<<(num_locks - 1) / key_block_size + 1, key_block_size>>>(
        qf, nitems, hashes, 0
    );
    ...
    bulk_get_cooperative<<<(nitems * 32 - 1) / bulk_block_size + 1, bulk_block_size>>>(
        qf, hashes, nitems, misses
    );
...
// modified version of buffers_provided - performs an initial bulk hash, should save work over other
// versions note: this DOES modify the given buffer - fine for all versions now This variant
// performs an ititial sort that allows us to save time overall as we avoid the atomic count-off and
// any sort of cross-thread communication
__host__ void bulk_insert(QF* qf, uint64_t nvals, uint64_t* keys, uint8_t flags) {
    ...
    // insert_from_buffers_hashed_onepass<<<(num_locks-1)/key_block_size+1, key_block_size>>>(qf,
    // num_locks, buffers, buffer_sizes);
    ...
    insert_from_buffers_hashed<<<(num_locks - 1) / bulk_block_size + 1, bulk_block_size>>>(
        qf, evenness
    );
...
__host__ void
bulk_insert_values(QF* qf, uint64_t nvals, uint64_t* keys, uint64_t* vals, uint8_t flags) {
    uint64_t key_block_size = 32;
    uint64_t key_block = (nvals - 1) / key_block_size + 1;
    // start with num_locks, get counts
    ...
    insert_from_buffers_hashed_values<<<(num_locks - 1) / bulk_block_size + 1, bulk_block_size>>>(
        qf, evenness, vals
    );
...
__host__ void bulk_insert_cooperative(QF* qf, uint64_t nvals, uint64_t* keys, uint8_t flags) {
    ...
    insert_from_buffers_cooperative<<<(32 * num_locks - 1) / key_block_size + 1, key_block_size>>>(
        qf, evenness
    );
...
__host__ void bulk_insert_reduce(QF* qf, uint64_t nvals, uint64_t* keys, uint8_t flags) {
    ...
    new_end = thrust::reduce_by_key(
        thrust::device, keys_ptr, keys_ptr + nvals, dupe_counts, thrust_keys, thrust_vals
    );
    ...
    // set_buffers_binary<<<(num_locks-1)/key_block_size+1, key_block_size>>>(qf, new_nvals,
    // slots_per_lock, new_keys, num_locks, buffers, flags);

    set_buffers_binary<<<(num_locks - 1) / key_block_size + 1, key_block_size>>>(
        qf, new_nvals, new_keys, flags
    );

    // set_buffer_lens<<<(num_locks-1)/key_block_size+1, key_block_size>>>(qf, new_nvals, new_keys,
    // num_locks, (uint64_t *) buffer_sizes, buffers);

    set_buffer_lens<<<(num_locks - 1) / key_block_size + 1, key_block_size>>>(
        qf, new_nvals, new_keys
    );
    ...
    insert_from_buffers_thrust<<<(num_locks - 1) / key_block_size + 1, key_block_size>>>(
        qf, evenness, new_keys, new_key_counts, new_nvals
    );
...
__host__ void
bulk_delete_values(QF* qf, uint64_t nvals, uint64_t* keys, uint64_t* vals, uint8_t flags) {
    uint64_t key_block_size = 32;
    uint64_t key_block = (nvals - 1) / key_block_size + 1;
    // start with num_locks, get counts
    ...
    // set_buffers_binary<<<(num_locks-1)/key_block_size+1, key_block_size>>>(qf, nvals,
    // slots_per_lock, keys, num_locks, buffers, flags);
    ...
    delete_from_buffers_hashed_vals<<<(num_locks - 1) / key_block_size + 1, key_block_size>>>(
        qf, evenness, vals
    );
...
__host__ void bulk_delete(QF* qf, uint64_t nvals, uint64_t* keys, uint8_t flags) {
    ...
    delete_from_buffers_hashed<<<(num_locks - 1) / key_block_size + 1, key_block_size>>>(
        qf, evenness
    );
...
__global__ void
bulk_get_nocount(QF* qf, uint64_t* vals, uint64_t nvals, uint64_t key_count, uint8_t flags) {
    uint64_t tid = threadIdx.x + blockDim.x * blockIdx.x;
...
__global__ void bulk_get_misses(
    QF* qf,
    uint64_t* vals,
    uint64_t nvals,
    uint64_t key_count,
...
__global__ void bulk_get_exact_misses(
    QF* qf,
    uint64_t* keys,
    uint64_t* vals,
    uint64_t nvals,
...
__global__ void
bulk_query_values_kernel(QF* qf, uint64_t* keys, uint64_t* vals, bool* hits, uint64_t nvals) {
    uint64_t tid = threadIdx.x + blockDim.x * blockIdx.x;
...
__host__ void
bulk_query_values(QF* qf, uint64_t* keys, uint64_t* vals, bool* hits, uint64_t nvals) {
    bulk_query_values_kernel<<<(nvals - 1) / 512 + 1, 512>>>(qf, keys, vals, hits, nvals);
}

__global__ void
bulk_get_kernel(QF* qf, uint64_t* vals, uint64_t nvals, uint64_t* returns, uint8_t flags) {
    uint64_t tid = threadIdx.x + blockDim.x * blockIdx.x;
...
__host__ uint64_t
bulk_get_exact_misses_wrapper(QF* qf, uint64_t* keys, uint64_t* vals, uint64_t nvals) {
    uint64_t* misses;
    // this is fine, should never be triggered
    cudaMallocManaged((void**)&misses, sizeof(uint64_t));
    cudaMemset(misses, 0, sizeof(uint64_t));

    bulk_get_exact_misses<<<(nvals - 1) / 512 + 1, 512>>>(
        qf, keys, vals, nvals, 1, misses, QF_NO_LOCK
    );
...
qf_set_count(QF* qf, uint64_t key, uint64_t value, uint64_t count, uint8_t flags) {
    if (count == 0)
        return 0;
...
__host__ __device__ int
qf_remove(QF* qf, uint64_t key, uint64_t value, uint64_t count, uint8_t flags) {
    if (count == 0)
        return true;
...
qf_count_key_value(const QF* qf, uint64_t key, uint64_t value, uint8_t flags) {
    if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
        if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
            key = aMurmurHash64A(((void*)&key), sizeof(key), qf->metadata->seed) %
                  qf->metadata->range;
...
qf_get_unique_index(const QF* qf, uint64_t key, uint64_t value, uint8_t flags) {
    if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
        if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
            key = aMurmurHash64A(((void*)&key), sizeof(key), qf->metadata->seed) %
                  qf->metadata->range;
...
int64_t
qf_iterator_from_key_value(const QF* qf, QFi* qfi, uint64_t key, uint64_t value, uint8_t flags) {
...
int qfi_next(QFi* qfi) {
    if (qfi_end(qfi))
        ...
        if (!is_runend(qfi->qf, qfi->current)) {
            ...
            uint64_t rank = bitrank(
                get_block(qfi->qf, block_index)->occupieds[0], qfi->run % QF_SLOTS_PER_BLOCK
            );
